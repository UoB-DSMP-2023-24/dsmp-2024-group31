...
class Personal_Account:
    ...
    def data_split_by_regular(self):
        ...
        data['transaction_num'] = data.groupby(['Account No', 'Date']).cumcount() + 1
        grouped = data.groupby(['Account No', 'Third Party Name',
                                'Amount', 'day'])

        monthly_counts = grouped['year_month'].nunique()

        periodic_criteria = monthly_counts[monthly_counts > 3].reset_index()


        # 标记周期性数据
        for _, row in periodic_criteria.iterrows():
            data.loc[(data['Account No'] == row['Account No']) &
                     (data['Third Party Name'] == row['Third Party Name']) &
                     (data['Amount'] == row['Amount']) , 'is_periodic'] = 1
        ...
        # 显示周期性数据和非周期性数据的前几行
        print(self.periodic_data)
        print(self.non_periodic_data.head())

    def monthly_feature_visualisation(self):
        if self.visualisation_flag['monthly_visualisation'] == 1:
            # 将日期转换为数值型（例如，从某个固定日期起的天数）
            reference_date = pd.to_datetime('2023-01-01')  # 可以选择一个合适的参考日期
            self.data['date_num'] = (self.data['Date'] - reference_date).dt.days
            for account in self.periodic_data['Account No'].unique():
                data = self.data[self.data['Account No'] == account]
                ...
                ax.set_xlabel('Date (days since 2023-01-01)')
    ...
    # for account in self.periodic_data['Account No'].unique():
    #     # 获取当前账户的数据
    #     account_data = self.data[self.data['Account No'] == account]
    #
    #     # 获取x, y, z值
    #     xs = account_data['date_num']
    #     ys = account_data['transaction_num']
    #     zs = account_data['Amount']
    #
    #     # 根据 is_periodic 为 1 的情况确定颜色
...
def main():
    csv_files = glob.glob('../SplitedData/Personal/*.csv')
