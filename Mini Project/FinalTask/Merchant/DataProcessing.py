...
class Merchant_transaction_frequency:
    ...
    def fit_kmeans(self, data):
        ...
        try:
            ...
            print(f"Successful clustering using {self.knee_locator} clusters")
    ...
    def data_categorisation(self):
        ...
        self.amount_frequency = self.data['Amount'].value_counts().reset_index()
        self.amount_frequency.columns = ['Amount', 'frequency']
        ...
        self.data_frequency_copy = pd.merge(self.data, self.amount_frequency, on='Amount')
    ...
    def data_result_show(self):
        for i in self.freq_dispersion:
            if i != 0:
                print("Data fluctuation anomalies")
                break
        print("Normal data fluctuations")
    ...
    def analysis_by_weekday(self):
        self.weekday_transactions = self.weekday_data.groupby('Day')['Amount'].agg(
            ['count', 'mean']).rename(columns={'count': 'Transactions', 'mean': 'Average Amount'}).reset_index()
        self.weekend_transactions = self.weekend_data.groupby('Day')['Amount'].agg(
            ['count', 'mean']).rename(columns={'count': 'Transactions', 'mean': 'Average Amount'}).reset_index()
        print("weekday_transactions:\n ",self.weekday_transactions)


        print(self.weekday_transactions['Transactions'].mean(), 'Number of weekday time-series transactions')
        print(self.weekday_transactions['Average Amount'].mean(), 'Amount of business day time-series transactions')
        print(self.weekend_transactions['Transactions'].mean(), 'Number of weekend time-series transactions')
        print(self.weekend_transactions['Average Amount'].mean(), 'Amounts traded in the weekend time series')
    ...
    def timedata_to_daynum(self):
        ...
        data_start=pd.to_datetime('2023.1.1')
        self.data["Day"]=self.data['Date']-data_start
    ...
    def split_by_weekday(self):
        ...
        print("weekday:\n",self.weekday_data['Date'])
        print("weekend:\n",self.weekend_data['Date'])

        self.analysis_by_weekday()

        # Group by date and calculate metrics
        # weekday_transactions = weekday_data.groupby('Date')['Amount'].agg(
        #     ['count', 'mean']).rename(columns={'count': 'Transactions', 'mean': 'Average Amount'})
        # weekend_transactions = weekend_data.groupby('Date')['Amount'].agg(
        #     ['count', 'mean']).rename(columns={'count': 'Transactions', 'mean': 'Average Amount'})
        #
    ...
    def amount_frequency_visualisation(self):
        ...
        self.amount_frequency.sort_values('Amount', inplace=True)

        # 绘制折线图
        plt.figure(figsize=(10, 6))
        plt.plot(self.amount_frequency['Amount'], self.amount_frequency['frequency'], marker='o',
                 color='blue')
    ...
    #         daily_frequency = self.data_frequency_copy.groupby('Date').size()
...
def main():
    CSV_files = glob.glob('../SplitedData/Retail/Tesco.csv')
